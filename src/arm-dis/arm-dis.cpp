			...
			bfd_boolean thumb)
{
  ...
  unsigned long mask;
  unsigned long value = 0;
  struct arm_private_data *private_data = (struct arm_private_data *) info->private_data;
...
#if 1
static void
arm_dis_print_address (bfd_vma addr, struct disassemble_info *info)
{
    info->fprintf_func(info->stream, "%#lx", addr);
}

static int
arm_dis_fprintf (void *stream, const char *format, ...)
{
  va_list ap;
  int rc;

  va_start(ap, format);
  rc = vfprintf((FILE *)stream, format, ap);
  va_end(ap);
...
{
  struct arm_private_data priv_data;
  struct disassemble_info dis_info;
  struct disassemble_info *info = &dis_info;
  int  size = 4;
  long          given;
  void          (*printer) (bfd_vma, struct disassemble_info *, long);

  memset(&priv_data, 0, sizeof(priv_data));
  priv_data.features.core = -1;

  memset(&dis_info, 0, sizeof(dis_info));
  dis_info.stream = stdout;
  dis_info.print_address_func = arm_dis_print_address;
  dis_info.fprintf_func = arm_dis_fprintf;
  dis_info.private_data = &priv_data;
  ...
  while (len > 0)
    {
      if (!is_thumb)
        ...
        is four bytes long and is either ordered 0123 or 3210.  */
          ...
          info->fprintf_func (info->stream, "%8x:\t%08x \t", pc, given);
        }
      else
        {
        ...
        in the first two bytes.  */
          ...
          if ((given & 0xF800) == 0xF800
              ...
              || (given & 0xF800) == 0xE800)
            {
              ...
              info->fprintf_func (info->stream, "%8x:\t%04x %04x\t", pc, given >> 16, given & 0xffff);
            }
          else
            {
              info->fprintf_func (info->stream, "%8x:\t%04x     \t", pc, given);
            }
        }
