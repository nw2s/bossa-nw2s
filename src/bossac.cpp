...
static Option opts[] =
{
	...
	{
		'F', "ftdi_simulate", &config.ftdiSimulate,
		{ ArgNone },
		"simulate FTDI 1200bps program mode hack"
	}
};
...
main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (config.ftdiSimulate)
        {
			printf("Resetting to boot loader\n");
			
			/* Connect at 1200 to tell the USB->Serial converter that it's time to erase and reset */
			SerialPort::Ptr port = portFactory.create(config.portArg);
			
            if (!port->open(1200))
            {
                fprintf(stderr, "No port to open on %s\n", config.portArg.c_str());
                return 1;
            }
