...
install32: $(BINDIR)\\bossa-nw2s-$(VERSION).msi
install64: $(BINDIR)\\bossa64-nw2s-$(VERSION).msi
.PHONY: install
...
install: strip
	tar cvzf $(BINDIR)/bossa-nw2s-$(MACHINE)-$(VERSION).tgz -C $(BINDIR) bossa$(EXE) bossac$(EXE) bossash$(EXE)
endif
...
COMMON_CXXFLAGS=-arch x86_64 -stdlib=libstdc++
#COMMON_CXXFLAGS=-arch i386
COMMON_LDFLAGS=-arch x86_64 -stdlib=libstdc++
#COMMON_LDFLAGS=-arch i386
APP=BOSSA-nw2s.app
DMG=bossa-nw2s-$(VERSION).dmg
VOLUME=BOSSA-nw2s
...
# CXX Flags
#
COMMON_CXXFLAGS+=-Wall -Werror -Wno-error=unused-variable -MT $@ -MD -MP -MF $(@:%.o=%.d) -DVERSION=\"$(VERSION)\" -g -O2
